...
import { BiBarcodeReader, BiFullscreen, BiSearchAlt } from "react-icons/bi";
import { AiFillCamera } from "react-icons/ai";
import LoadingOverlay from 'react-loading-overlay';
import { useAuth } from "contexts/AuthContext";
import ScanBarcode from './staff/ScanBarcode.js';
  ...
  useEffect(
    () => {
      async function fetchProducts() {
        ...
        await axios.get(product.showStoreProducts + "/" + id).then((response) => {
  ...
  return (
    <>
      ...
      <div className="content">
        ...
        <div >
          <Button className="btn-fill" onClick={() => { setBarcodeOnly(!barcodeOnly) }} style={{ marginBottom: "5px", padding: '10px' }} color="primary" type="submit">
            Scan Barcode: {barcodeOnly ? "true" : "false"}<BiBarcodeReader size={30} />
          </Button>
          <Button className="btn-fill" disabled style={{ marginBottom: "5px", padding: '10px' }} color="primary" type="submit">
            Scan With Camera<AiFillCamera size={30} />
          </Button>
          <Button className="btn-fill" onClick={() => { setSearchOnly(!searchOnly) }} style={{ marginBottom: "5px", padding: '10px' }} color="primary" type="submit">
            Search Only: {searchOnly ? "true" : "false"}<BiSearchAlt size={30} />
          </Button>
          <Button className="btn-fill" onClick={handle.enter} style={{ marginBottom: "5px", padding: '10px' }} color="primary" type="submit">
            Enter fullscreen<BiFullscreen size={30} />
          </Button>
                ...
                <LoadingOverlay active={requestLoading} spinner text='Loading your request...'>
                  {!sellDetailsMode ?
                    <Card>
                      ...
                      {searchOnly ? null : <ScanBarcode cart={cart} setCart={setCart} />}
