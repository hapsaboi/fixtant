...
function Staff() {
        ...
        async function fetchStaffs() {
          await axios.get(staff.showStoreStaff).then((response)=>{
            if(response.data.status===true){
              ...
              setNotificationDetails({msg:"Error Loading Staffs, Please Referesh The Page", type:"danger"});
  ...
  async function updateStaff () {
    await axios.patch(staff.updateStaff,currentStaff).then((res)=>{
      if(res.data.status){
        setNotificationDetails({msg:"Staff Updated Successfully", type:"success", change:res.data.change});
      }
      else{
  ...
  const result = staffs.map(({_id,name,email,phone}) => ({_id,name,email,phone}));
      ...
      <div className="content">
        <Row>
          <Col md="12">
            ...
            <Card>
              <CardHeader>
                <CardTitle  className='pull-left' tag="h4">Staff</CardTitle> 
              ...
              {dataload===false?
                      ...
                      <tbody>
                        {search(result).map((row,key) => (
                          <tr key={key}>
                            ...
                            <td>
                              <div>
                                <Button onClick={()=> selectStaff(row._id)} className="btn-fill" style={{marginBottom:"5px"}} color="primary" type="submit">
                                  Show
                                </Button>
                              </div>
                            </td>
                ...
                :
                  <div style={{color: "#39B54A", textAlign: "center",padding:"20px"}}> 
                    ...
                    <CardTitle tag="h4">Nothing To Show Yet... Add Some Staffs to The System</CardTitle>
                  </div>  
                } 
                </>
              :
            ...
            :
              <Button style={{width:"100%",marginBottom:'15px'}} onClick={()=>setLoading(!loading)} className="btn-fill" color="primary">
                <FiArrowLeft size={20} /> <font style={{paddingLeft:"30px"}}>Back To Staffs </font>
              </Button>
            }
          </Col>
          <Col md="12">
                  ...
                  (  
                    <CardBody>
                            ...
                            <Col className="pl-md-1" md="6">
                              <FormGroup>
                                ...
                                <Input
                                  ...
                                  onChange={(e) => setCurrentStaff({...currentStaff, upassword:e.target.value})}
                                />
                              </FormGroup>
                            </Col>
                          </Row>
                      ...
                      <Row>
                        ...
                        <Col md="6">
                        <Button onClick={()=>updateStaff()} className="btn-fill" style={{width:"100%"}} color="primary" type="submit">
                          Update Staff
                        </Button>
                        </Col>
                      </Row>
